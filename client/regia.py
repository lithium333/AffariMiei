...
def drwFrame():
	global window
	window = tk.Frame(windowParent,bg="#B0E0FF",height = hparam, width = 1070)
	...
	if(not jdata[7]):
		...
		lb.place(x=100,y=40,w=250)
		lr = tk.Label(window, bg='white', width=20, font="15",text='PACCHI ROSSI:')
		lr.place(x=400,y=40,w=250)
	else:
		lb = tk.Label(window, bg='white', width=20, font="15",text='CONTRADE:')
		lb.place(x=100,y=40,w=550)
	la = tk.Label(window, bg='white', width=20, font="15",text='PLAY AUDIO:')
	la.place(x=700,y=40,w=270)
	...
	if(jdata[7]):
		...
		for cnt in range(0,jdata[11]):
			...
			arr_cbox_b[cnt].place(x=100,y=100+cnt*30,h=30,w=250)
		...
		for cnt in range(0,jdata[11]):
			...
			arr_cbox_r[cnt].place(x=400,y=100+cnt*30,h=30,w=250)
		...
		if(jdata[3]!=None):
			...
			ent_offer.place(x=400,y=hposOffer,h=30,w=250)
			
		# SEPARATORE 1 SOLO CON MOD PACCHI
	...
	abutn=0
	abutv=[]
	dynFrameTxt = tk.Text(window,bg="#FFFF00")
	dynFrameTxt.place(x=700,y=100,h=300+maxextrarows*30,w=250)
	dynsb = tk.Scrollbar(window, command=dynFrameTxt.yview)
	dynsb.place(x=950,y=100,h=300+maxextrarows*30,w=20)
	dynFrameTxt.configure(yscrollcommand=dynsb.set)
	dynFrame = tk.Frame(dynFrameTxt, bg="#FF7F00", width = 250)
	for riga in acfg_file:
		riga = riga.rstrip()
		args = riga.split(",")
		# DISABLED: print outside dynamic box
		# exec("abutv.append((master=window, bg='black', fg='white', font='12',text=args[0], command=lambda: playaudio(\""+relpath+"/sound/"+args[1]+"\")))")
		# abutv[abutn].place(x=700,y=100+abutn*30,h=30)
		# NEW: print inside dynamic box
		exec("abutv.append(tk.Button(master=dynFrame, height=1, bg='black', fg='white', font='12',text=args[0], command=lambda: playaudio(\""+relpath+"/sound/"+args[1]+"\")))")
		abutv[abutn].place(x=10,y=10+abutn*30,h=30,w=230)
		# button count
		abutn+=1
	dynFrame.config(height=20+abutn*30)
	#dynFrame.place(x=700,y=100)
	dynFrameTxt.window_create("end", window=dynFrame)
	#dynFrameTxt.insert("end", "\n")
...
windowParent.geometry('1070x'+hparam)
